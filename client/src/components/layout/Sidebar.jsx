...
import {
  LayoutDashboard, User, Scroll, Ghost, Sword, Castle,
  Timer, Target, BookOpen, Brain, BarChart3, Users,
  Settings, LogOut, X, Zap, CheckSquare, Trophy, Calendar,
} from 'lucide-react';
...
export default function Sidebar({ open, onClose }) {
  const { hunter, clearAuth } = useHunterStore();
  ...
  return (
    <>
      <aside className="hidden lg:flex flex-col w-60 bg-slate-950/95 border-r border-slate-800/60 shrink-0">
        <SidebarContent hunter={hunter} rankColor={rankColor} onLogout={handleLogout} />
      </aside>
      <AnimatePresence>
        {open && (
          <motion.aside initial={{ x: -260 }} animate={{ x: 0 }} exit={{ x: -260 }}
            ...
            <button onClick={onClose} className="absolute top-4 right-4 p-1.5 text-slate-500 hover:text-slate-300"><X size={18} /></button>
            <SidebarContent hunter={hunter} rankColor={rankColor} onLogout={handleLogout} onNav={onClose} />
          </motion.aside>
        )}
      </AnimatePresence>
    </>
...
function SidebarContent({ hunter, rankColor, onLogout, onNav }) {
  return (
    <>
      <div className="p-5 border-b border-slate-800/60">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="font-display text-xs text-cyan-400 tracking-widest">SOLO LEVELING</p>
            <p className="font-heading text-[10px] text-slate-500 tracking-wider uppercase">The System</p>
          </div>
        </div>
      </div>
      ...
      {hunter && (
        <div className="mx-3 mt-3 p-3 rounded-xl bg-slate-900/60 border border-slate-800/60">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-cyan-500/30 to-purple-500/30 border border-cyan-500/30 flex items-center justify-center shrink-0">
              <span className="font-display text-sm text-cyan-400">{hunter.hunterName?.[0]?.toUpperCase()}</span>
            </div>
            <div className="min-w-0">
              <p className="font-heading font-semibold text-sm text-slate-100 truncate">{hunter.hunterName}</p>
              <p className={`font-display text-[10px] tracking-wider ${rankColor}`}>{hunter.rank} RANK · LV.{hunter.level}</p>
            </div>
          </div>
          <div className="mt-2.5 xp-bar-track">
            <div className="xp-bar-fill" style={{ width: `${hunter.xpProgress || 0}%` }} />
          </div>
        </div>
      )}
      ...
      <nav className="flex-1 overflow-y-auto no-scrollbar py-3 px-2">
        {NAV_ITEMS.map((item, i) => {
          if (item.divider) return <div key={i} className="divider mx-2 my-1" />;
      ...
      <div className="p-3 border-t border-slate-800/60">
        <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg font-heading text-sm text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all">
